...
<body>
      ...
      if (isset($_POST['signup-btn'])) {
            ...
            if (!empty(trim($_POST['email'])) && !empty(trim($_POST['pass']))) {
                  $authenticated = signup($users, $_POST['name'], $_POST['email'], $_POST['pass']);
                  if ($authenticated == 1) {
      ...
      <main>
            <form action="<?= $_SERVER['PHP_SELF'] ?>" class="signup" method="post">
                  <h1>Signup</h1>
                  <div class="mb-3">
                        <label for="name" class="form-label">Nombre</label>
                        <input type="name" class="form-control" id="name" name="name" value='<?= $authenticated == -1 || $authenticated == 2 ? '' : $_POST['name'] ?>'>
                  </div>
                  <div class="mb-3">
                  ...
                  ?>
                  <div>
                        <input type="submit" class="btn btn-primary mb-3" value="Registrar" name="signup-btn">
                        <a href="./login.php" class="btn btn-primary mb-3">Ya tengo un usuario</a>
                  </div>
            </form>
      </main>
</body>
